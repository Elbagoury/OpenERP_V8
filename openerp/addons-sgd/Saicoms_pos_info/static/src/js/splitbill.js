function openerp_restaurant_splitbill(instance, module){
    ...
    module.SaicomsAddInfoScreen = module.ScreenWidget.extend({
        template: 'SaicomsAddInfo',
        show_leftpane:   false,
        previous_screen: 'products',
        init: function(parent, options) {
            console.log('in init'+' '+options.order.get('name') + '  '+options.test_info);
            var self = this;
        ...
        show: function(){
            console.log('check in POS Payment show');
            this._super();
            var self = this;
        ...
        save: function(){
            var gift_card = this.$(".gift_card").attr('value');
            var perform_user_id = this.$(".perform_user_id").attr('value');
            var advisory_id = this.$(".advisory_id").attr('value');
            var pos_source = this.$(".pos_source").attr('value');
            var has_friend = $('input[name=has_friend]:checked').val();
            var customer_user_rating = this.$(".customer_user_rating").attr('value');
            var customer_comment = this.$(".customer_comment").attr('value');
            var customer_rating = $('input[name=customer_rating]:checked').val()

            this.order = this.pos.get('selectedOrder');

            this.order.perform_user_id = perform_user_id;
            this.order.advisory_id = advisory_id;
            this.order.pos_source = pos_source;
            this.order.has_friend = has_friend;
            this.order.customer_user_rating = customer_user_rating;
            this.order.customer_rating = customer_rating;
            this.order.customer_comment = customer_comment;
        },
        renderElement: function(){
            ...
            var clientline_html = QWeb.render('SaicomsAddInfo',{widget: this, order:order,test_info:'WWFFFAA'});
    ...
    module.PosWidget.include({
        build_widgets: function(){
            ...
            this._super();
            var order = this.pos.get('selectedOrder');
            this.splitbill_screen = new module.SaicomsAddInfoScreen(this,{order:order});
