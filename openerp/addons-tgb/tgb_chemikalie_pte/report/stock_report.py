...
class Parser(report_sxw.rml_parse):
    ...
    def get_customer_po_no(self, sale_name):
        customer_po_no = ''
        if sale_name:
            sale_obj = self.pool.get('sale.order')
            sale_ids = sale_obj.search(self.cr, self.uid, [('name','=',sale_name)])
            if sale_ids:
                customer_po_no = sale_obj.browse(self.cr, self.uid, sale_ids[0]).customer_po_no
