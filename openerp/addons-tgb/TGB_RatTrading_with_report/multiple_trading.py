...
class wizard_rat_trading(osv.osv_memory):
    _name = 'wizard.rat.trading'
    _columns = {
        'name': fields.char(string='Name', size=255),
        'created_by': fields.many2one('res.users', string='Created By'),
        'date_import': fields.datetime(string='Date'),
        'trading_detail_ids': fields.one2many('wizard.rat.trading.detail', 'parent_id', 'Detail Quotation'),
    }
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(wizard_rat_trading, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type,
                                                                     context=context, toolbar=toolbar, submenu=False)
        return res

    def default_get(self, cr, uid, fields, context=None):
        res = super(wizard_rat_trading, self).default_get(cr, uid, fields, context=context)
        active_id = context.get('active_id', False)
        print 'active id',active_id , context
        return True
        ...
        if customer_ids and len(customer_ids)>0:
            for id in ids:
                for customer in customer_ids:
                    new_line = self.pool.get('wizard.rat.trading.detail').create(cr,uid,{'parent_id':id,
    ...
    def confirm_quotation(self,cr,uid,ids,context=None):
        ...
        print 'here we return result', result
        return result

class wizard_rat_trading_detail(osv.osv_memory):
    _name = 'wizard.rat.trading.detail'
    _columns = {
    ...
    def onchange_product(self,cr,uid,ids,product,context=None):
        print 'check in on change product'
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if context is None: context = {}
        res = super(wizard_rat_trading_detail, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type,
                                                                     context=context, toolbar=toolbar, submenu=False)
        ...
        print 'context in field view get', context
