            ...
            for item in order.trading_detail_ids:
                for product in product_list_ids:
                    if item['product_'+str(product)] !=0:
                        total+= item['product_'+str(product)]*item['product_'+str(product)+'_price']
            res[order.id] = total
        return res

    _columns = {
        'name': fields.char(string='Name', size=255, readonly=True, states={'draft': [('readonly', False)],},),
        'created_by': fields.many2one('res.users', string='Created By', readonly=True, states={'draft': [('readonly', False)],},),
        'date_import': fields.datetime(string='Date', readonly=True, states={'draft': [('readonly', False)],},),
        'trading_detail_ids': fields.one2many('rat.trading.detail', 'parent_id', 'Detail Quotation', readonly=True, states={'draft': [('readonly', False)],},),
